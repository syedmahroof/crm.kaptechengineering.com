...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
                <i class="fas fa-clipboard-list mr-3 text-indigo-600 dark:text-indigo-400"></i>Create Visit Report
            </h1>
            <p class="mt-2 text-gray-600 dark:text-gray-400">Record a new visit report for projects, customers, or contacts</p>
        </div>
    ...
    <form action="{{ route('visit-reports.store') }}" method="POST" class="space-y-6">
        @csrf
                ...
                <div>
                    ...
                    <select name="project_ids[]" id="project_ids" multiple
                        ...
                        @foreach($projects as $proj)
                            <option value="{{ $proj->id }}" {{ (old('project_ids') && in_array($proj->id, old('project_ids'))) || (isset($project) && $project->id == $proj->id) ? 'selected' : '' }}>{{ $proj->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="customer_ids[]" id="customer_ids" multiple
                        ...
                        @foreach($customers as $cust)
                            <option value="{{ $cust->id }}" {{ (old('customer_ids') && in_array($cust->id, old('customer_ids'))) || (isset($customer) && $customer->id == $cust->id) ? 'selected' : '' }}>{{ $cust->name }}</option>
                        @endforeach
                    </select>
                    <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                ...
                <div>
                    ...
                    <select name="contact_ids[]" id="contact_ids" multiple
                        ...
                        @foreach($contacts as $cont)
                            <option value="{{ $cont->id }}" {{ (old('contact_ids') && in_array($cont->id, old('contact_ids'))) || (isset($contact) && $contact->id == $cont->id) ? 'selected' : '' }}>{{ $cont->name }}@if($cont->email) ({{ $cont->email }})@endif</option>
                        @endforeach
                    </select>
                    <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="report" id="report" rows="6"
                              ...
                              placeholder="Enter detailed report of the visit, observations, discussions, and any important notes...">{{ old('report') }}</textarea>
        ...
        <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <button type="submit" class="px-6 py-2.5 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-lg shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
                <i class="fas fa-save mr-2"></i>Create Visit Report
            </button>
        </div>
    </form>
</div>
