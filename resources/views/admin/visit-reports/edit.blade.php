...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Edit Visit Report</h1>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <form action="{{ route('visit-reports.update', $visitReport->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="project_id" id="project_id" required
                        ...
                        @foreach($projects as $proj)
                            <option value="{{ $proj->id }}" {{ old('project_id', $visitReport->project_id) == $proj->id ? 'selected' : '' }}>{{ $proj->name }}</option>
                        @endforeach
                    </select>
                    @error('project_id')
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="report" id="report" rows="6"
                              ...
                              placeholder="Enter daily or visiting time report update">{{ old('report', $visitReport->report) }}</textarea>
                    @error('report')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
