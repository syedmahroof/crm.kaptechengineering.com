...
@section('title', 'Create District/City')

@section('content')
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Create District/City</h1>
        <a href="{{ route('districts.index', ['country_id' => $selectedCountryId, 'state_id' => $selectedStateId]) }}" class="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <form action="{{ route('districts.store') }}" method="POST">
                ...
                <div>
                    ...
                    <select name="country_id" id="country_id" required
                        ...
                        @foreach($countries as $country)
                            <option value="{{ $country->id }}" {{ old('country_id', $selectedCountryId) == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                        @endforeach
                    </select>
                    @error('country_id')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="mt-6 flex justify-end space-x-3">
                <a href="{{ route('districts.index', ['country_id' => $selectedCountryId, 'state_id' => $selectedStateId]) }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">
                    Cancel
                </a>
