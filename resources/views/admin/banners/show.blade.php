...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $banner->title }}</h1>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-6">
        <div>
            ...
            <p class="mt-1 text-gray-900 dark:text-white">{{ $banner->title }}</p>
        </div>

        @if($banner->description)
            <div>
                <label class="text-sm font-medium text-gray-500 dark:text-gray-400">Description</label>
                <p class="mt-1 text-gray-900 dark:text-white">{{ $banner->description }}</p>
            </div>
        @endif
        ...
        @if($banner->image_url)
            <div>
                ...
                <img src="{{ $banner->image_url }}" alt="{{ $banner->title }}" class="mt-2 w-full h-64 object-cover rounded-lg">
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <p class="mt-1 text-gray-900 dark:text-white">{{ $banner->sort_order }}</p>
            </div>
        </div>

        <div>
            <a href="{{ route('admin.banners.edit', $banner->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
